-- ============================================
-- BASE DE DONNÉES COMPLÈTE - ÉCHEC-PONG HYBRIDE
-- Version parallèle corrigée
-- ============================================
...
CREATE TABLE joueurs (
    ...
    -- NOUVEAUX CHAMPS POUR CONCEPTION PARALLÈLE
...
-- Table des collisions balle-pièce (ancienne version)
...
-- NOUVELLE TABLE : Collisions précises (pour conception parallèle)
...
-- NOUVELLE TABLE : Statistiques de défense
...
-- NOUVELLE TABLE : Préférences joueur
...
INSERT INTO terrains (
    ...
    'Arena Parallèle', 2.74, 1.525, 2.74, 1.0, 8, 2, 'bleu'
);
...
INSERT INTO configuration_pieces (
    ...
    'Parallèle Classique',
    'Configuration pour la conception parallèle avec défense par colonne',
    3, 2, 2, 1, 1, 1,
    '{
        "regle_avantage": true,
        "points_par_set": 11,
...
-- Insérer des préférences pour les joueurs de test
...
-- Vue pour les matchs en cours avec informations parallèles
...
-- Vue pour les statistiques de précision des joueurs
...
SELECT 
    ...
    ROUND(AVG(cp.precision_tir) * 100, 2) as precision_tir_moyenne,
    ROUND(AVG(CASE WHEN cp.colonne_visee = cp.colonne_atteinte THEN 1.0 ELSE 0.0 END) * 100, 2) as precision_colonne
FROM joueurs j
...
-- Vue pour les défenses par colonne
...
SELECT 
    ...
    ROUND(sd.taux_reussite * 100, 2) as taux_reussite_pourcent,
    CASE 
...
-- Vue pour les pièces vulnérables (colonne non protégée)
...
SELECT 
    ...
    p.colonne_protegee,
    CASE 
        WHEN p.colonne_protegee THEN 'PROTÉGÉE'
        WHEN p.rangee = 0 THEN 'AVANT - VULNÉRABLE'
        ELSE 'ARRIÈRE'
...
-- Fonction pour initialiser les pièces avec colonnes parallèles
...
BEGIN
    ...
    -- Pièces pour le joueur Sud (rangée 0 = avant, 1 = arrière)
    INSERT INTO piece_echecs (id_match, id_joueur, type_piece, colonne, rangee, nombre_vies, vies_restantes, valeur)
    VALUES 
    -- Pions (rangee 0 - avant)
...
-- Fonction pour vérifier si une colonne est défendue
...
BEGIN
    -- Récupérer la position du joueur dans le match
    -- Note: Cette fonction nécessite une table temporaire ou une jointure
    -- Pour simplifier, on retourne FALSE par défaut
    RETURN FALSE;
END;
$$ LANGUAGE plpgsql;

-- Fonction pour enregistrer une collision précise
...
BEGIN
    ...
    -- Mettre à jour les statistiques de défense si défense tentée
...
-- Trigger pour mettre à jour la durée du match
...
BEGIN
    IF NEW.statut = 'mort' AND OLD.statut != 'mort' THEN
        ...
        -- Déterminer si c'est le joueur nord ou sud
...
-- Trigger pour mettre à jour les préférences
...
COMMENT ON TABLE terrains IS 'Configurations des terrains de jeu parallèles';
COMMENT ON TABLE joueurs IS 'Informations des joueurs avec statistiques parallèles';
COMMENT ON TABLE match_hybride IS 'Matchs avec système de colonnes parallèles';
COMMENT ON TABLE piece_echecs IS 'Pièces d''échecs avec protection par colonne';
COMMENT ON TABLE balle IS 'Balles avec prédiction de sortie par colonne';
COMMENT ON TABLE coup_pingpong IS 'Coups avec précision de colonne';
COMMENT ON TABLE collision_precise IS 'Collisions précises par colonne et rangée';
COMMENT ON TABLE configuration_pieces IS 'Configurations des pièces';
COMMENT ON TABLE statistiques_defense IS 'Statistiques de défense par colonne';
COMMENT ON TABLE preferences_joueur IS 'Préférences des joueurs';

COMMENT ON COLUMN piece_echecs.colonne_protegee IS 'Indique si la pièce est protégée par la raquette du joueur';
...
COMMENT ON COLUMN coup_pingpong.colonne_atteinte IS 'Colonne réellement atteinte';

-- ==================== FIN ====================

DO $$
BEGIN
    RAISE NOTICE 'Base de données "Échec-Pong Parallèle" créée avec succès!';
    RAISE NOTICE 'Tables créées: 10 tables';
    RAISE NOTICE 'Index créés: 22 index';
    RAISE NOTICE 'Vues créées: 4 vues';
    RAISE NOTICE 'Fonctions créées: 4 fonctions';
    RAISE NOTICE 'Triggers créés: 3 triggers';
    RAISE NOTICE '';
    RAISE NOTICE 'Configuration parallèle activée:';
    RAISE NOTICE '  - Défense par colonne';
    RAISE NOTICE '  - Prédiction de trajectoire';
    RAISE NOTICE '  - Collisions précises';
    RAISE NOTICE '  - Statistiques de défense';
END $$;